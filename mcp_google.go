...
func addGoogleTools(reg *toolRegistrar, client *Client) {
	...
	addTool(reg, client, "campaign_criteria",
		"List the criteria attached to a campaign (geo targets, languages, ad-schedule windows, negative keywords), each with the campaignId~criterionId that google_remove_entity takes. Geo and language criteria include constant_id and resolved constant_name alongside negative and status. Optionally filtered by campaign_criterion.type.",
		runCampaignCriteria)
